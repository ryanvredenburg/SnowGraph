...
function getSeasonalData(triplet, elementCd, callback) {
  ...
  endDate = moment(lastSeasonYear(endDate).toString() + '-05-31'  )
  ...
  var prevTwo = []
  var prevThree = []
  var finalYear = lastSeasonYear(endDate)
  ...
  soap.createClient(url, function(err, client) {
      client.getData(args, function(err, result) {
          ...
          snotelData.dateTime = new Array(snotelData.values.length)
          ...
          var seasonData = new Array(4)
          seasonData[0] = new Array()
          seasonData[1] = new Array()
          seasonData[2] = new Array()
          seasonData[3] = new Array()
        
          var seasonTitles = new Array(4)
...
function getForecastData(lat, long, callback){
 request('https://api.darksky.net/forecast/'+ process.env.DARK_SKY +'/'+lat+','+long+'?extend=hourly', { json: true }, (err, res, body) => {
  ...
  var forecastData = {daily: new Array()}
    //process daily 
